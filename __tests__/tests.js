...
['Redis', 'Memory', 'Sequelize'].forEach((store) => {
  let acl = null;
  ...
  describe(store, () => {
    ...
    beforeAll((done) => {
      ...
      if (store === 'Memory') {
        ...
        console.log(`====> connecté a la db RedisStore`)
        done();
      } else if (store === 'MongoDB') {
        MongoDB.connect('mongodb://127.0.0.1:27017/aclify', (error, db) => {
          acl = new Acl(new MongoDBStore('mongo', 'acl_'));
          console.log(`====> connecté a la db MongoDB`)
          done();
        });
        ...
        sequelize.authenticate()
          .then(() => {
            ...
            console.log(`====> connecté a la db sequelize`)
            done();
          });
      }
    });
